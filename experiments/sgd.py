...
class SGD:
    ...
    def train(self, X_training_data):
        ...
        # training_data = np.dot(np.dot(X_training_data['data_tfidf'], self.__u), np.linalg.inv(np.diag(self.__s)))
        # predicted_y = self.__model.predict(X_training_data['data_tfidf'])
        ## predicted_y = self.__model.predict(training_data)
    ...
    def test(self, X_test_data):
        ...
        """
        for input, prediction, label in zip(X_test_data['data'], predicted_y, X_test_data['labels']):
            ...
            if prediction == label:
                ...
                print("[[ label : ", label, " | prediction : ", prediction, " | text : ", input)
